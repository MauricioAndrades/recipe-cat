...
cachedModules[7378] = {
	...
	}.call(this, cachedModules[7378], cachedModules[7378].exports), cachedModules[5665] = {
		exports: {}
	},
	function(e, r) {
		var s = require("express"),
		...
		o.get("", function(e, r) {
			...
			t.get("https://spoonacular-recipe-food-nutrition-v1.p.mashape.com/recipes/findByIngredients", {
				...
				for (var s = e.data, a = 0; a < s.length; a++) r.title = s[a].title, r.id = s[a].id, r.image = s[a].image;
				r.render(e.data)
			})["catch"](function(e) {
				e instanceof Error ? console.log("Error", e.message) : e.json(data)
			})
		}), e.exports = o
	}.call(this, cachedModules[5665], cachedModules[5665].exports);
...
var logger = require("morgan"),
	...
	routes = cachedModules[7378].exports,
	ingredients = cachedModules[5665].exports,
	app = express(),
	hbs = exphbs.create({
		helpers: {
			foo: function() {
...
})), app.set("view engine", "handlebars"), app.use(logger("dev")), app.use(bodyParser.json()), app.use(bodyParser.urlencoded({
	extended: !1
})), app.use(cookieParser()), app.use(express["static"](path.join(__dirname, "public"))), app.use("/", routes), app.use("/ingredients", ingredients), app.use(function(e, r, s) {
